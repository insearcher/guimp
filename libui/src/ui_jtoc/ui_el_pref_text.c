...
/*   Updated: 2019/07/08 04:30:56 by sbecker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_text_params(t_ui_el *e, t_jnode *n)
{
	...
	if ((tmp = jtoc_node_get_by_path(n, "text_area.max_text_size")) && tmp->type == number)
		e->text_area->max_text_size = jtoc_get_int(tmp);
	...
	if ((tmp = jtoc_node_get_by_path(n, "text_area.params")) && tmp->type == string)
	{
		hash = ft_strhash(jtoc_get_string(tmp));
